 ...
 *
 * This is a plain Node.js version (no tsx) that works in the VM
 */
...
function generateLessonScript(lesson, course) {
  const scripts = {
...
// Parse the TypeScript file (simple regex-based parsing for course structure)
function parseCourseContent() {
  const courseFile = join(projectRoot, 'lib', 'data', 'course-content.ts')
  const content = readFileSync(courseFile, 'utf-8')

  // This is a simplified parser - in production, you'd use proper TypeScript AST parsing
  // For now, we'll extract basic information using regex

  const scripts = []

  // Extract course titles
  const courseTitleRegex = /title:\s*['"]([^'"]+)['"]/g
  const courseMatches = [...content.matchAll(courseTitleRegex)]

  // For demo purposes, create scripts for first few lessons
  const sampleLessons = [
    {
      id: 'lesson-react-1-1',
      title: 'Welcome & Course Overview',
      description: 'Get started with Advanced React Patterns and understand what you will learn',
      courseTitle: 'Advanced React Patterns'
    },
    {
      id: 'lesson-react-1-2',
      title: 'Compound Components',
      description: 'Learn how to create flexible and reusable component APIs using the Compound Components pattern',
      courseTitle: 'Advanced React Patterns'
    },
    {
      id: 'lesson-react-1-3',
      title: 'Flexible Compound Components',
      description: 'Advanced techniques for building even more flexible compound component systems',
      courseTitle: 'Advanced React Patterns'
    },
    {
      id: 'lesson-react-2-1',
    ...
    },
    {
      ...
      description: 'Deep dive into advanced TypeScript type system features',
      courseTitle: 'TypeScript Mastery'
    }
  ]

  sampleLessons.forEach(lesson => {
    const script = generateLessonScript(lesson, { title: lesson.courseTitle })
...
// Main execution
console.error('📚 Extracting lesson scripts from course content...')

const scripts = parseCourseContent()

// Write to file
const outputDir = join(projectRoot, 'temp')
mkdirSync(outputDir, { recursive: true })

const outputPath = join(outputDir, 'lesson-scripts.json')
...
console.error('')
console.error('Sample lessons included:')
scripts.forEach(s => {
  console.error(`  • ${s.lessonId}: ${s.title}`)
})
